  ...
  public class PartModel
  {
    ...
    public string EncodeToStr()
    {
      ...
      for (int i = 0; i < Prefabs.Count; i++)
        Result += string.Format("{0}:{1}:{2}:{3}:{4}", Prefabs[i].Id, Prefabs[i].AppearanceIndex,
    ...
    public List<SerializedPart> DecodeToStr(string customStr)
    {
      ...
      for (int i = 0; i < Data.Length - 1; i++)
      {
        ...
        {
          Id = int.Parse(Args[0]),
          AppearanceIndex = int.Parse(Args[1]),
          Position = ParseToSerializedVector3(ToVector3(Args[2])),
          Rotation = ParseToSerializedVector3(ToVector3(Args[3])),
          Scale = ParseToSerializedVector3(ToVector3(Args[4]))
        });
      }
